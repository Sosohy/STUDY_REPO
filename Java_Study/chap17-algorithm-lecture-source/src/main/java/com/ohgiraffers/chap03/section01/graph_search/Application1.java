...
/* 필기
 *  깊이우선탐색(Dept-First Search)
 *  후입선출 구조에 stack 또는 재귀함수를 활용한다.
 *  한쪽 분기를 정하여 최대 깊이까지 탐색을 마친 후
...
public class Application1 {
    ...
    /* 설명. 문제에서 요구하는 입력 값을 받아 감염된 컴퓨터의 갯수(int/Integer) 반환하는 메소드 작성 */
    public static Integer solution(String input) throws IOException {
        ...
        /* 노드와 간선 정보 담길 map 작성(node의 갯수에 기반한 int형 2차원 배열) */
        map = new int[node + 1][node + 1];       // node의 번호와 인덱스 번호 일치시키기 위해 크기 1 증가

        /* 방문 배열 작성(방문 배열을 작성하는 이유는 방문했던 노드를 다시 stack에 담지 않거나 다시 재귀함수 호출을 하지 않기 위함) */
        visit = new boolean[node + 1];

        /* 인접리스트(map)로 그래프에 대한 정보 작성 */
        for (int i = 0; i < edge; i++) {         // 간선정보 처리할 반복문
            StringTokenizer str = new StringTokenizer(br.readLine());

            int a = Integer.valueOf(str.nextToken());
            int b = Integer.valueOf(str.nextToken());

            /* 무방향 그래프로 처리되기 위해 노드 번호들(인덱스 번호)을 반대로도 적용한다. */
//            map[a][b] = 1;
//            map[b][a] = 1;
            map[a][b] = map[b][a] = 1;
        }
    ...
    private static void dfs(int start) {
        ...
        /* 설명. 넘어온 노드와 연관된 노드를 찾는 반복문(기존에 방문하지 않았던 노드) */
