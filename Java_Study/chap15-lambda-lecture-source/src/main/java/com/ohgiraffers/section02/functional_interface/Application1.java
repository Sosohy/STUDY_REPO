package com.ohgiraffers.section02.functional_interface;
...
public class Application1 {
    public static void main(String[] args) {
         ...
         *  5. Predicate : 매개변수와 boolean값을 반환하는 testXXX()를 지니고 있다.(매개변수를 활용해서 boolean 반환)
        * */
        Consumer<String> consumer = str -> System.out.println(str + " 입력");
        consumer.accept("hello");

        BiConsumer<String, LocalTime> biConsumer =
                (str, time) -> System.out.println(str + "가 " + time + "에 입력");
        biConsumer.accept("hello", LocalTime.now());

        ObjIntConsumer<Random> objIntConsumer =
                (random, bound) -> System.out.println("0부터 " + bound + "전까지 난수 발생 : " + random.nextInt(bound));
