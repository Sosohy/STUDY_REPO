...
public class LoggingAspect {
     ...
     *  포인트컷(pointcut)은 여러 조인포인트들에 어드바이스를 적용할 곳을 지정한 것이다.
     ...
     *   => com.ohgiraffers.section01.advice 패키지 및 하위 패키지에 속해 있고 이름이 stu로 시작하는 파라미터가 0개 이상인 모든 메소드이며
     *      접근제어자와 반환형은 상관 없음
    * */
...
//        System.out.println("After Advice 동작");
    ...
    /* 설명. throwing 속성 값이 매개변수명과 일치해야 한다.(exception) */
    ...
    /* 5. Around Advice */
    /* 설명
     *  이 어드바이스는 조인 포인트를 완전히 장악하기 때문에 앞서 살펴본 어드바이스 모두
     *  Around 어드바이스로 조립할 수 있다.
     *  심지어 원본 조인 포인트를 언제 실행할지, 실행 자체를 안할지, 계속 실행할지 여부까지도 제어한다.
     *  AroundAdvice의 조인포인트 매개변수는 ProceedingJointPoint로 고정되어 있다.
