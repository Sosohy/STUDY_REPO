...
public class ResolverController {
    ...
    /* 설명. 서블릿 때와 마찬가지로 파라미터를 활영하지 않고서는 리다이렉트를 통해 값을 전달할 수 없다. -> 스프링은 해법이 있음 */
    @GetMapping("string-redirect")
    public String stringRedirect(Model model){
        model.addAttribute("message1", "문자열로 뷰어를 반환하며 리다이렉트");

        return "redirect:/?message1=sdf";     // main페이지로 이동함
    }
    ...
    /* 설명. 단순 포워드 시에는 String 반환과 Model을 활용하는 코드와 ModelAndView를 활용한 코드가 차이가 없다. */
